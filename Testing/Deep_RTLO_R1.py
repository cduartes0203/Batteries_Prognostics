...
class RTLO:
    def __init__(self, nI, nR, nO, n_layers=1, ηS=[0.1, 0.1, 0.1], τ=10, mode='past', act='tanh'):
        np.random.seed(42)
        ...
        # Estados ocultos e ativações para cada camada
        self.hP = [0.1 * np.ones(nR) for _ in range(n_layers)]
        self.hU = [0.1 * np.ones(nR) for _ in range(n_layers)]
        self.hL = [0.1 * np.ones(nR) for _ in range(n_layers)]
        
        self.hP2 = [0.1 * np.ones(nR) for _ in range(n_layers)]
        self.hU2 = [0.1 * np.ones(nR) for _ in range(n_layers)]
        self.hL2 = [0.1 * np.ones(nR) for _ in range(n_layers)]
        
        self.uP = [np.zeros(nR) for _ in range(n_layers)]
        self.uP2 = [np.zeros(nR) for _ in range(n_layers)]
        
        # Traços de sensibilidade para cada camada
        self.pS = [np.zeros((nR, nR)) for _ in range(n_layers)]
        self.qS = [np.zeros((nR, nI + 1 if l == 0 else nR + 1)) for l in range(n_layers)]

        # Pesos de entrada/inter-camadas:
        # Camada 0: recebe x (tamanho nI+1)
        # Camadas L > 0: recebem o estado oculto anterior + bias (tamanho nR+1)
        self.wI = []
        self.wI.append(XavierUniform([nR, nI + 1], sd=42))
        for l in range(1, n_layers):
            self.wI.append(XavierUniform([nR, nR + 1], sd=42 + l))

        # Pesos recorrentes para cada camada
        self.wR = [XavierUniform([nR, nR], sd=41 + l) for l in range(n_layers)]
        
        # Pesos de saída conectados à última camada oculta
        self.wO = XavierUniform([nO, nR], sd=40)
        
        # Matrizes de feedback de erro para cada camada
        self.BS = [XavierUniform([nR, nO], sd=39 + l) for l in range(n_layers)]
        ...
        self.wR_hist = [[] for _ in range(n_layers)]
        self.wI_hist = [[] for _ in range(n_layers)]
        self.wO_hist = []
    ...
    def fit(self, xP, yR):
        ...
        # Passagem direta (Forward Pass) por todas as camadas ocultas
        for l in range(self.n_layers):
        ...
        # Saída baseada no estado da última camada
        yP = self.wO @ new_hP[-1]
        eS = yR - yP

        # Atualização dos traços e pesos de cada camada
        curr_x = xP
        for l in range(self.n_layers):
            ...
            δRS = η2 * np.outer((self.BS[l] @ eS), np.ones(self.nR)) * self.pS[l]
        ...
        # Atualização dos pesos da camada de saída (conectada à última camada oculta)
        δOS = η1 * np.outer(eS, new_hP[-1])
        self.wO += δOS
        self.wO_hist.append(self.wO.flatten())

        # Atualização do estado armazenado
