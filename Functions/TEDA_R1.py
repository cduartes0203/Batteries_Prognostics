...
class AutoCloud:
	def __init__(self,m=3,nS=1,nI=1,nR=1,nO=1,ηS=[0.1,0.1,0.1],
			  #N1=1,N2=1,N3=1,
	...
	def run(self,X,MergeGrnls=True,MergeOffGrnls=False,FitOffGrnls=True,OfflineRun=True):
		...
		if self.k==1:
			...
			for cloud in self.c:
				...
				n1 = n
				n2 = n
				R = math.sqrt((((m**2)+1)*n2*variance/n1)-variance)
				if n < m**2: m = math.sqrt(n)
				elif n >= m**2: m=m

				if(norm_eccentricity<=(m**2 +1)/(2*n)):
			...
			for cloud in self.c:
				...
				norm_typicality = typicality/(self.k-2)
				cloud.eccAn = eccentricity
				R = math.sqrt((((m**2)+1)*n*variance/n)-variance)
